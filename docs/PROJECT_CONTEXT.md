...
Instead, we estimate Meta revenue as **its spend x the real return rate seen on Google + Bing
(~4.75x)**, giving a sensible **~$930K**. We label it clearly as an estimate.
...
- *"How do you handle Meta having no revenue?"* - Estimate it as spend x the blended Google+Bing return rate (~4.75x); labelled as an estimate. Avoids a $146M fake-sales error.
