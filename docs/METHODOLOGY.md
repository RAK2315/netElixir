...
```
meta_revenue = meta_spend x assumed_ROAS
...
(`objective="quantile"`, alpha = 0.1 / 0.5 / 0.9) -> conditional P10/P50/P90 of window
...
`E = max(p10 - y, y - p90)` and widen the band by their `(1-alpha)` empirical
...
- **Meta revenue is imputed** (spend x assumed ROAS) - modelled, not measured.
